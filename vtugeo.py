...
"""
vtugeo aims to convert  2D (x-y) vtu unstructured grids to geodataframe structures and vice versa. Furthermore, output as polygon shapefile is provided
All node and element fields from vtu are transfered to the geodataframe and can be written out to shapefile as attributes. Node based fields
...
def unique_nd(a):
    ...
    inspired by https://stackoverflow.com/questions/8560440/removing-duplicate-columns-and-rows-from-a-numpy-2d-array
    """
...
def vtu_to_geo(fctn_inpt='testmesh.vtu',inpt_flfrmt='vtu-ascii',srs_crs='EPSG:31468',elmt_type='triangle',Output=None):
    """
    ...
    #get triangles nodes, keep in mind that quads could exist as well, not covered yet
    if len(mesh.cells.keys())==1:
        try:
            mesh_nds=mesh.cells[elmt_type]
        except:
            sys.exit('Only 2D Meshes consisting of triangles only are supported at moment')
    else:
        sys.exit('More than one type of elements not supported')
        
    #Get the Nodes corresponding to the elements https://www.geeksforgeeks.org/python-accessing-all-elements-at-given-list-of-indexes/
    nds_cord = list(itemgetter(*mesh_nds)(mesh.points))
    tri_geom = [shapely.geometry.Polygon(zip(nds_cord[i][:,0],nds_cord[i][:,1],nds_cord[i][:,2]))for i in range(0,len(nds_cord))]   
    #next we create the dataframe for the element fields
    df_elmnt_flds=pd.DataFrame(mesh.cell_data[elmt_type]);
    
    #finally for the node_based information
    #get the NodalValues as Elementbased attributes with particular column for each node
    meshpnt_fldnms=list(mesh.point_data.keys())
    #create the dataframe
    df_pnt_flds = pd.DataFrame() #create a dataframe
    
    #we have to define the order of the nodes in a polygon , in vtk it is counter clockwise, 
    #however after a shape is written out this turns to be clockwise
    ...
    for meshpnt_fldnm in meshpnt_fldnms:
        fld_data=np.stack(itemgetter(*mesh_nds)(mesh.point_data[meshpnt_fldnm]))
        df_pnt_flds[['N_'+str(i)+'_'+meshpnt_fldnm for i in NodeOrder]] = pd.DataFrame(fld_data)
    print('... done writing fields')
    #Finally we merge all the dataframes to one geodataframe    
    #create a geopandas dataframe
    gdf_mesh = gpd.GeoDataFrame(pd.concat([df_elmnt_flds,df_pnt_flds], axis=1), geometry=tri_geom,crs=srs_crs)
    if isinstance(Output,str):
        gdf_mesh.to_file(Output)
    
    return gdf_mesh

def geo_to_vtu(fctn_inpt='testmesh.shp',elmt_fldnms_long=['MaterialIDs','bulk_element_ids','bulk_face_ids'],pnt_fldnms_long=['bulk_node_ids'],Output='outputmsh.vtu',Output_flfrmt='vtu-binary'):
    """
    This Function takes a geodataframe/shapefile and converts it back to a VTU mesh
    ...
    elmt_fldnms_long and pnt_fldnms_long define some (more than 10 digit) OGS field names which we need later to replace
    CHECK BY INDEX
    @author: Erik
    """
    if isinstance(fctn_inpt, str):
    ...
    #extract cell_data arrays, which by definiton neither start with "N_" or geometry
    cell_fld_nms=[col_nm for col_nm in gdf_inpt.columns if col_nm.startswith('N_') is False and col_nm !='geometry']
    print ('...cell fieldnames ',cell_fld_nms,' have been identified')
    #extract the nodal_array_nms and reshape datasets    
    #We check for all point_datafields and append them to the existing numpy array
    pnt_fld_uniqs = set()
    ...
    #%% Extract all information from the shape which are relevant for the VTU Mesh
    #first we want the coordiantes for all nodes forming a polygon (mesh element)
    ...
    df_pnt_cell_flds=pd.DataFrame(np.vstack( df_pnt_cell_flds_lst),columns=clm_nms)
    #create a new dataframe in which all fields which belong to element values are repeated two times
    df_cell_flds=pd.concat([gdf_inpt[cell_fld_nms]]*pnts_per_elmnt).sort_index()
    #append the cell fld to the pnt_cell_flds
    df_pnt_cell_flds=pd.concat([df_pnt_cell_flds,df_cell_flds.reset_index(drop=True)],axis=1)
    #We check for all point_datafields and append them to the existing numpy array
    for pnt_fld_uniq in pnt_fld_uniqs:
        node_array_nms=['N_'+str(i)+'_'+pnt_fld_uniq for i in range(0,dimension)]
    ...
    df_uniq_pts=pd.DataFrame(unique_nd(df_pnt_cell_flds[dim_clm_name].to_numpy()),columns=dim_clm_name)
    df_uniq_pts['Node_ID']=np.linspace(0,len(df_uniq_pts)-1,len(df_uniq_pts))
    #merge the uniquepoints with the element based dataframe in order to get the NodeID for each point in each element
    df_pnt_cell_flds = pd.merge(df_uniq_pts,df_pnt_cell_flds,how="inner", on=dim_clm_name)
    ...
    for pnt_fld_uniq in pnt_fld_uniqs:
        ...
        #we check whether the cell_array name needs to be replaced (10 digit issue in shape)
        try:
    ...
    #Next we create the msh cell dictionary and the array which points element to node    
    #sort by element id
    df_msh_cells=df_pnt_cell_flds.sort_values(by=['Element_ID'])
    #get node index per cell
    msh_cell_pntids_arr=df_msh_cells['Node_ID'].to_numpy().reshape(int(np.max(df_pnt_cell_flds['Element_ID']))+1,3).astype(int)
